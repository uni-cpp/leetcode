...
{
public:
    uint32_t numIslands( std::vector< std::vector< char > >& grid );

private:
    void dfs( std::vector< std::vector< char > >& grid, uint32_t row, uint32_t column );

private:
    uint32_t m_rows{ 0U };
    uint32_t m_columns{ 0U };
    std::vector< std::vector< bool > > m_is_visited{ };
};
